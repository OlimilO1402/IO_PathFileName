...
Public Function AddBytes(bArray() As Byte) As Variant
    Dim ByteSize As Long
    On Local Error GoTo AddBytesError
    
    If Not blInit Then Initialize
    
    ByteSize = UBound(bArray) - LBound(bArray) + 1
    On Local Error GoTo 0
    
    CallWindowProc VarPtr(m_CRC32Asm(0)), _
                   VarPtr(m_CRC32), _
                   VarPtr(bArray(LBound(bArray))), _
                   VarPtr(m_CRC32Table(0)), _
                   ByteSize
...
Public Function CalculateBytes(bArray() As Byte) As Variant
    Clear
    CalculateBytes = AddBytes(bArray)
End Function

Public Function CalculateString(Text As String) As Variant
...
Public Function TestChecksum(Text As String, CheckSum As Long) As Boolean
    If CheckSum = CalculateString(Text) Then
        TestChecksum = True
    End If
End Function
